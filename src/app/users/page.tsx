...
export default function Page() {
  ...
  const handleSearch = async (event: FormEvent) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API}/panel/users/${query}`,
      {
        headers: {
    ...
    setTargetUser({
      ...data,
      discriminator: data.discriminator.toString().padStart(4, "0"),
    });
    setSaveData({
      ...data,
      discriminator: data.discriminator.toString().padStart(4, "0"),
  ...
  const handleSave = async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API}/panel/users/${targetUser.id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div>
      {JSON.stringify(targetUser) != JSON.stringify(saveData) && (
      ...
      <div className="py-4 px-4 md:px-10 flex justify-center">
        {!targetUser ? (
          <div>Search for a user to get started</div>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-full">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Edit{" "}
                    {`${targetUser.display_name}'s avatar.`}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center justify-center">
                    <div className="w-fit h-fit relative">
                      ...
                      <img
                        src={
                          saveData.avatar == targetUser.avatar
                            ? Formatting.avatar(
                                targetUser.id,
                                targetUser.avatar
                              )
                      ...
                      />
                      {targetUser.avatar != saveData.avatar && (
                        <Button
              ...
              <Card className="h-fit">
                <CardHeader>
                  ...
                  <CardDescription>
                    Edit{" "}
                    {`${targetUser.display_name}'s profile.`}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-1">
                    ...
                    <Input
                      onChange={(event) =>
                        setSaveData({ ...saveData, email: event.target.value })
                      }
                      placeholder={Formatting.censorEmail(targetUser.email)}
                      id="email"
                    />
                  </div>
                  <div className="space-y-1">
                    <Label htmlFor="global_name">Global Name</Label>
                    <Input
                      onChange={(event) =>
                        setSaveData({
                          ...saveData,
                          global_name: event.target.value,
                        })
                      }
                      placeholder={targetUser.global_name ?? "None Set"}
                      id="global_name"
                    />
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder={targetUser.discriminator}
                      maxLength={4}
                      id="discriminator"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-5 gap-x-2 gap-y-4">
                    {badgeMap.map((badge, key) => (
                      <button
                        ...
                        style={{
                          background:
                            saveData.flags & badge.key ? "#14532d" : "#7f1d1d",
                        }}
                      >
                        {badge.value}
                      </button>
              ...
              <Card className="h-fit">
                ...
                <div>
                    ...
                    <p>{new Date(targetUser.created_at).toLocaleString()} (<ReactTimeago date={targetUser.created_at} className="flex-shrink-0 text-sm" />)</p>
                  </div>
                  <div className="mt-2">
                    <Label>About Me</Label>
                    <p>{targetUser.about_me || "No about me set."}</p>
                  </div>
                  <div className="mt-2">
                    <Label>Banned</Label>
                    <p>{targetUser.banned ? "Yes" : "No"}</p>
                  </div>
                  <div className="mt-2">
                    <Label>Presence</Label>
                    <p>Online: {targetUser.presence.online ? "Yes" : "No"} <br/> Status: {targetUser.presence.status.charAt(0).toUpperCase() + targetUser.presence.status.slice(1)}</p>
                  </div>
              ...
              <Card className="h-fit">
                <CardHeader>
                  ...
                  <CardDescription>Perform actions on {targetUser.display_name}.</CardDescription>
                </CardHeader>
                <CardContent className="inline-block justify-center">
                  <Button className="danger mr-2 my-1">Ban User</Button>
                  <Button className="danger mr-2 my-1">Suspend User</Button>
                  <Button className="danger my-1">Delete User</Button>
                </CardContent>
              </Card>
